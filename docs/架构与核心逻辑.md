...
atomicIsRunning = false
  → 不调用 Thread.cancel()（启动前取消可能跳过 worker 的退出信号）
...
- 暂停菜单中的 Cheat List 支持多行金手指的增删改及启停。`EmulatorCore.replaceCheats` 等待模拟线程退出后调用桥接层，先解析整个新列表，全部成功才替换；移除旧集合前撤销 ROM patch 和 master-code hook。
- `CheatStore` 按稳定的 ROM 文件名保存到 `Documents/Cheats/<ROM文件名>.json`；磁盘保存失败回滚核心列表，重新进入游戏时恢复。C 桥接包含生成的 `mgba/flags.h`，保证与静态库的条件编译结构体布局一致。
...
**即时状态 `.state`** 是模拟器运行状态快照，不需要游戏本身提供保存菜单。文件接口调用 `mCoreSaveStateNamed` / `mCoreLoadStateNamed`，使用 `SAVESTATE_ALL & ~SAVESTATE_CHEATS`，金手指由当前 Cheat List 独立管理，不随历史即时状态恢复。
